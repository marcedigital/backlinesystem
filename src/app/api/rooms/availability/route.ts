...
async function handleGetAvailability(req: NextRequest) {
  try {
    ...
    const roomsNeedingSync = rooms.some(
      (room) => room.googleCalendarSyncEnabled
    );
    ...
    for (const room of rooms) {
      ...
      if (room.googleCalendarSyncEnabled && calendarService) {
        try {
          ...
          const nextDayEvents = await calendarService.getEvents(
            room.id,
            startOfNextDay,
            endOfNextDay
          );
          ...
          logToConsole(
            "error",
            `Error fetching Google Calendar events for room ${room.id}:`,
            calendarError
          );
      ...
      const currentDaySlots = generateTimeSlots(
        startOfDay,
        24,
        unavailableTimes
      );
      const nextDaySlots = generateTimeSlots(
        startOfNextDay,
        6,
        unavailableTimes
      );
